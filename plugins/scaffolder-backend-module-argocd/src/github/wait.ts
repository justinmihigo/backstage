...
import { ScmIntegrationRegistry, GithubCredentialsProvider } from "@backstage/integration";
...
export const githubWaitLastWorkflow  = (options: {
    ...
    return createTemplateAction<{
        ...
        async handler(ctx) {
            const token = integrations?.github!.byHost('github.com')?.config.token || options.config?.getOptionalString('integrations.github.token') || (await options.credentials?.getCredentials({ url:'github.com' }))?.token;
            const octokit = new Octokit({ auth: token });
            ctx.logger.info(`Querying the last commit on ${ctx.input.branch} repository ${ctx.input.owner}/${ctx.input.repo}`);
